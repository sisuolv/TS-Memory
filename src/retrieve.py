...
class Retriever():
    ...
    def search(
        ...
        drop_self_eps: float = 1e-8,
    ):
        if query_vector.ndim == 1:
            query_vector = query_vector.reshape(1, -1)
        # Retrieve candidates (k + 1), then optionally filter self-matches by distance.
        k_search = top_k + 1
        if params is None:
            distances, indices = self.index.search(query_vector, k_search)
        else:
            distances, indices = self.index.search(query_vector, k_search, params=params)

        if drop_self:
            # Filter exact/near-exact matches (typically self-retrieval) based on distance.
            # Keep shape (B, top_k); if not enough neighbors remain, distances will contain inf.
            distances = distances.astype(np.float64, copy=True)
            distances[distances <= drop_self_eps] = np.inf
            ...
            if np.isinf(distances).any():
                warnings.warn(
                    f"drop_self removed too many neighbors: some queries have <{top_k} "
                    f"neighbors after filtering dist<={drop_self_eps}.",
                    RuntimeWarning,
                )
        else:
            # Backward compatible behavior: return k neighbors, optionally drop the first.
            if drop_first:
        ...
        for var_idx, var_name in enumerate(variable_names):
            ...
            for batch_idx in tqdm(range(batch_num)):
                ...
                if end_idx_batch[-1] <= border2s[0]:
                    ...
                    distances_batch, boundary_idx_batch, timestamp_idx_batch = retriever.search(
                        query_vector_batch,
                        top_k=top_k,
                        drop_self=drop_self,
